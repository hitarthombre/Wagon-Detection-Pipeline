...
def process_integrated_pipeline(video_path, model_name='yolov8n.pt', 
                                blur_threshold=100.0, conf_threshold=0.25,
                                enable_ocr=True, use_gpu=True, save_output=True):
    """
    Complete integrated pipeline with all enhancements:
    - Manual confidence score display for YOLO detections
    - Play/pause controls (p/r/q)
    ...
    print(f"\nVideo: {width}x{height} @ {fps} FPS, Total frames: {total_frames}")
    ...
    while True:
        if not paused:
            ...
            current_display_frame = final_frame.copy()
        ...
        # Display
        cv2.imshow('Integrated Pipeline - All Steps', current_display_frame)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if os.path.exists(default_video):
            ...
            print("Usage: python integrated_pipeline.py <video_file_path> [options]")
            print("\nOptions:")
            ...
            print("\nExample:")
            print("  python integrated_pipeline.py video.mp4 yolov8n.pt 100 0.25 True True True")
            print("\nControls:")
            print("  'p' - Pause video")
            print("  'r' - Resume video")
            print("  'q' - Quit")
    ...
    process_integrated_pipeline(video_path, model_name, blur_threshold, 
                               conf_threshold, enable_ocr, use_gpu, save_output)
